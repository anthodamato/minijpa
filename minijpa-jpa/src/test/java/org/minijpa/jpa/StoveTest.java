	...
	public void nativeQuery() {
		...
		try {
			...
			Query query = em.createNativeQuery(
					"select * from stove where induction" + PersistenceUnitProperties.getTrueCondition());
			...
			query = em.createNativeQuery(
					"select * from stove where induction" + PersistenceUnitProperties.getFalseCondition());
			stoves = query.getResultList();

			Assertions.assertEquals(2, stoves.size());
			CollectionUtils.containsAll(stoves, Arrays.asList(s_4, s_8));

			query = em.createNativeQuery("update stove set model='New Model' where induction"
					+ PersistenceUnitProperties.getTrueCondition());
			int count = query.executeUpdate();
			Assertions.assertEquals(1, count);

			query = em.createNativeQuery(
					"delete from stove where induction" + PersistenceUnitProperties.getTrueCondition());
