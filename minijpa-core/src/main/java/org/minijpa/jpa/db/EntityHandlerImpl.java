...
public class EntityHandlerImpl implements EntityHandler {
    ...
    public EntityHandlerImpl(PersistenceUnitContext persistenceUnitContext, EntityContainer entityContainer,
...
//	LOG.debug("findById: primaryKey={}", primaryKey);
        Optional<ModelValueArray<FetchParameter>> optional = jdbcQueryRunner.findById(metaEntity, primaryKey, lockType);
        if (optional.isEmpty())
            return null;
...
//	}
    ...
    public Object queryVersionValue(MetaEntity metaEntity, Object primaryKey, LockType lockType) throws Exception {
        Optional<ModelValueArray<FetchParameter>> optional = jdbcQueryRunner.runVersionQuery(metaEntity, primaryKey,
                lockType);
        if (optional.isEmpty())
            return null;
...
//		    metaEntity, joinColumnMapping.getAttribute(), fk, lockType);
    ...
    private void loadJoinTableRelationships(Object parentInstance, MetaEntity entity, MetaAttribute attribute,
        ...
        if (attribute.getRelationship().isOwner()) {
            ...
            Object result = jdbcQueryRunner.selectByJoinTable(pk, entity.getId(), attribute.getRelationship(),
                    attribute, this);
    ...
    private Object loadRelationshipByForeignKey(Object parentInstance, MetaEntity entity,
        ...
        LOG.debug("loadRelationshipByForeignKey: foreignKeyAttribute={}; foreignKeyValue={}", foreignKeyAttribute,
                foreignKeyValue);
...
//		    parentInstance, parentInstance.getClass(), foreignKeyAttribute, foreignKeyInstance, entity);
    ...
    public Object loadAttribute(Object parentInstance, MetaAttribute a, Object value) throws Exception {
        ...
        LOG.debug("loadAttribute: to Many targetAttribute={}; relationship.getJoinTable()={}", targetAttribute,
        ...
        if (relationship.getJoinTable() == null) {
            ...
            return jdbcQueryRunner.selectByForeignKey(entity, relationship.getOwningAttribute(), parentInstance,
                    LockType.NONE, this);
        }
    ...
    protected void update(MetaEntity entity, Object entityInstance, ModelValueArray<MetaAttribute> modelValueArray)
            throws Exception {
        // It's an update.
        ...
        if (MetaEntityHelper.hasOptimisticLock(entity, entityInstance)) {
            ...
            parameters.addAll(MetaEntityHelper.convertAVToQP(entity.getVersionAttribute().get(), currentVersionValue));
        }
    ...
    protected void insert(MetaEntity entity, Object entityInstance, ModelValueArray<MetaAttribute> modelValueArray)
            throws Exception {
        Pk pk = entity.getId();
...
//  LOG.info("Primary Key Generation Strategy: " + pkStrategy);
    ...
    private void updatePostponedJoinColumnUpdate(MetaEntity entity, Object entityInstance) throws Exception {
...
//      LOG.info("persist: attributeInstance.getClass()={}", attributeInstance.getClass());
            ...
            for (RelationshipJoinTable relationshipJoinTable : relationshipJoinTables) {
                if (relationshipJoinTable.getOwningEntity() == entity
                    ...
                    Optional<RelationshipJoinTable> optional = joinTables.stream()
                            .filter(j -> j.getTableName().equals(relationshipJoinTable.getTableName())).findFirst();
                    if (optional.isEmpty())
    ...
    private void removeJoinTableRecords(MetaEntity entity, Object primaryKey, List<QueryParameter> idParameters)
            throws Exception {
        ...
        for (RelationshipJoinTable relationshipJoinTable : relationshipJoinTables) {
            jdbcQueryRunner.removeJoinTableRecords(entity, primaryKey, idParameters, relationshipJoinTable);
        }
    }
