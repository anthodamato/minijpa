...
public class EntityWriterImpl implements EntityWriter {
    ...
    public EntityWriterImpl(PersistenceUnitContext persistenceUnitContext, EntityContainer entityContainer,
            SqlStatementFactory sqlStatementFactory, DbConfiguration dbConfiguration, EntityLoader entityLoader,
...
//	checkOptimisticLock(entity, entityInstance, idValue);
        ...
        SqlUpdate sqlUpdate = sqlStatementFactory.generateUpdate(entity, columns, idColumns,
...
//	LOG.info("Primary Key Generation Strategy: " + pkStrategy);
        if (pkStrategy == PkStrategy.IDENTITY) {
            ...
            SqlInsert sqlInsert = sqlStatementFactory.generateInsert(entity, columns, true, pkIndex == -1,
                    Optional.of(entity), persistenceUnitContext.getTableAliasGenerator());
            String sql = dbConfiguration.getSqlStatementGenerator().export(sqlInsert);
            String[] ids = { pk.getAttribute().getColumnName() };
            Object pkId = null;
//	    try {
            pkId = dbConfiguration.getJdbcRunner().insertReturnGeneratedKeys(connectionHolder.getConnection(), sql,
